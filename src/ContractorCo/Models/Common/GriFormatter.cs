...
public class GriFormatter<T>
{
    public GriFormatter(IEnumerable<T> data)
    {
    ...
    private IEnumerable<string> FormatRows(int columnsCount, int gap) =>
        this.FormatRows(this.GetColumnsWidths(columnsCount),new string(' ', gap));

    public IEnumerable<string> FormatRows(int[] columnWidths, string gap)
    {
    ...
    {
        int columnsCount = this.GetColumnsUpperBound(width, gap);
    ...
    private int[] GetColumnsWidths(int columnsCount)
        => this.GetColumns(columnsCount, 0, 0, false).columnWidths;
    
    private int GetColumnsUpperBound(int width, int gap)
    {
